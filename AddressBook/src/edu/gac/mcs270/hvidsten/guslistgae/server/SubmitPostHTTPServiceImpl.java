...
public class SubmitPostHTTPServiceImpl extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		EntryData editingPost = AddressListModel.getEditingPost();
		if(editingPost !=null) {
			if(blobKey != null)
			...
			AddressListModel.updateEditedPost(editingPost, name, title, descr, 
					                            address, price, url);
		}
		else {
			//Map the ImageURL to the blobservice servlet, which will serve the image
			url = "/guslist/blobservice?blob-key=" + blobKey.getKeyString();
			EntryData post = new EntryData(title,descr,price, address,
